...
def _buscar_freight_id_por_numero(numero_nota, manifesto_numero, token, dominio_esl):
    """
    ...
    """
    url = f"https://{dominio_esl}/api/invoice_occurrences"
    headers = {"Authorization": f"Bearer {token}"}
    params = {"manifest_id": str(manifesto_numero), "per": 50}
...
def enviar_baixa_minuta_task(self, baixa_id):
    ...
    try:
        ...
        if not freight_id and manifesto and manifesto.manifesto_id_tms:
            ...
            freight_id = _buscar_freight_id_por_numero(
                nf.numero_nota, 
        ...
        URL_ESL_FRETE = f"https://{config.dominio_esl}/api/v1/freights/{freight_id}/invoice_occurrences"
