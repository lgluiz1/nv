...
async function atualizarListaViva(numeroManifesto) {
    try {
        ...
        // --- 2. LOGICA DA SINCRONIZAÇÃO LÁ EM CIMA (O QUE EU TINHA COMIDO) ---
        // Atualizamos o badge de status antes da análise de mudança para o motorista ver que o app "leu" o servidor
        if (contador) {
        ...
        // Se os dados forem idênticos, paramos aqui, mas deixamos o badge de status atualizado
        if (cacheDadosRaw === novosDadosJSON) {
            console.log("ℹ️ Sem alterações. Mantendo tela.");
            // Opcional: Voltar para o badge de contagem após 1s se nada mudou
            setTimeout(() => { renderizarContadoresAposSinc(contador, notas); }, 1000);
            return;
        }

        if (areaDinamica && notas.length > 0) {
            // 1. INJEÇÃO DA BUSCA (Se não existir) - MANTIDO 100% SEU
            ...
            // 3. RENDERIZAÇÃO DAS NOTAS PENDENTES
            let htmlPendentes = '';
            ...
            // --- RESTANTE DA LÓGICA (COLETIVA, KM, ETC) ---
            renderizarContadoresAposSinc(contador, notas, totalFinalizadas, numeroManifesto, grupos, containerColetiva);
            
            filtrarNotasOffline();

            // 5. SALVAMENTO DO CACHE FINAL
...
// FUNÇÃO AUXILIAR PARA NÃO REPETIR CÓDIGO DE CONTADORES
function renderizarContadoresAposSinc(contador, notas, totalFinalizadas, numeroManifesto, grupos, containerColetiva) {
    if (!contador) return;
    
    // Reaproveita sua lógica original de contadores
    let htmlContadores = `<div class="d-flex gap-2 justify-content-center"><span class="badge bg-secondary p-2">${notas.length} Notas no Manifesto</span>`;
    ...
    // Atualiza Baixa Coletiva se necessário
    if (grupos && containerColetiva) {
        const transfPendentes = grupos['TRANSFERENCIA'].filter(n => !n.ja_baixada);
        if (transfPendentes.length > 0) {
        ...
        } else { containerColetiva.innerHTML = ''; }
    }
}
// =====================================================
