...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (authenticated) {
        ...
        // Adicionado listener para o novo input de câmera nativa
...
// =====================================================
// NOVA LOGICA DE CÂMERA (SUBSTITUI AS FUNÇÕES ANTIGAS)
// =====================================================
...
/**
 * Processa a foto tirada pelo aplicativo nativo do celular
 */
function handleCameraNativa(event) {
    ...
    reader.onload = function(e) {
        const img = new Image();
        img.onload = function() {
            // Alta resolução para garantir leitura pela ESL
...
async function salvarRegistro() {
    ...
    if (temFoto) {
        // Gera o blob em alta qualidade (0.85) para integração com ESL
...
async function atualizarDadosHeader() {
    try {
        ...
        if (data && data.nome) {
            document.getElementById('header-nome-motorista').textContent = data.nome.split(' ')[0];
        }
    } catch (e) {
