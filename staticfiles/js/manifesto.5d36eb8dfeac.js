...
async function ligarCameraFull() {
    try {
        ...
        // Esconde o modal para não atrapalhar a tela cheia
        // (Opcional, ou apenas coloque o z-index da camera maior)
        document.getElementById('full-screen-camera').style.display = 'block';
    } catch (err) {
...
function capturarFotoFull() {
    const video = document.getElementById('video-full');
    const canvas = document.getElementById('canvas-preview'); // O canvas que está dentro do modal
    const ctx = canvas.getContext('2d');
    
    // Define o tamanho da foto baseado no vídeo
    canvas.width = video.videoWidth;
    canvas.height = video.videoHeight;
    ctx.drawImage(video, 0, 0);
    
    // Interface: Esconde placeholder e mostra o canvas (substituindo a foto antiga se houver)
    document.getElementById('placeholder-camera').style.display = 'none';
    canvas.style.display = 'block';
    
    // Fecha a câmera e volta para o modal
    fecharCameraFull();
}

async function tentarQualquerCamera() {
    try {
        ...
        alert("Não foi possível encontrar nenhum dispositivo de vídeo disponível.");
    }
}
function tirarFoto() {
    const video = document.getElementById('video-camera');
...
/**
 * Função chamada pelo botão "Dar Baixa" na lista de notas.
 * Resolve o erro: Uncaught ReferenceError: abrirModalBaixa is not defined
 */
function abrirModalBaixa(numeroNota, chaveAcesso) {
    console.log("Abrindo baixa para NF:", numeroNota, "Key:", chaveAcesso);

    const tituloEl = document.getElementById('modal-titulo-nf');
    const inputChave = document.getElementById('hidden-chave-nf');

    // Validação de segurança para evitar erro de 'null'
    if (!tituloEl || !inputChave) {
        alert("Erro técnico: Elementos do modal não encontrados no HTML.");
        return;
    }

    // Armazena a KEY que será enviada para a ESL
    tituloEl.innerText = `Ocorrência NF-e ${numeroNota}`;
    inputChave.value = chaveAcesso;

    // Reset de interface da câmera
