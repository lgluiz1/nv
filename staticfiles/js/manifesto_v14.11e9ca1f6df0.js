...
async function atualizarListaViva(numeroManifesto) {
    try {
        ...
        if (areaDinamica && notas.length > 0) {
            ...
            // 1. Aplica o filtro de busca se houver algo digitado
            filtrarNotasOffline();

            // 2. SALVAMENTO DO CACHE (O SEGREDO DA VELOCIDADE)
            // Pegamos o HTML que acabamos de montar e guardamos no "bolso" do navegador
            if (areaDinamica && areaDinamica.innerHTML.length > 100) { 
                ...
                console.log("💾 Cache atualizado para carregamento instantâneo.");
            }
