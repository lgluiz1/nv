...
async function handleManifestoSearch(event) {
    ...
    try {
        ...
        if (response.ok) {
            // 2. Requisição OK (202 Accepted). Celery está processando.
            document.getElementById('loadingModalLabel').textContent = 'Sucesso! Processando dados...';
            
            // Aguarda 3 segundos para o Celery terminar o requests.get e salvar no DB
            setTimeout(() => {
                loadingModal.hide();
                // 3. Verifica o status novamente para renderizar a lista
                checkManifestoStatus(data.mensagem, 'success'); 
            }, 3000);

        } else {
            // Erro de validação ou motorista já ativo (400 Bad Request)
            throw new Error(data.mensagem || 'Erro desconhecido na busca.');
        }

    } catch (error) {
        loadingModal.hide();
        // 4. Se falhar, volta para a tela de busca com a mensagem de erro
