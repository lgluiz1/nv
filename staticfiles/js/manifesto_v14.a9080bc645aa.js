...
async function executarBaixaOp(chave, tipo, isCompleto) {
    ...
    try {
        ...
        if (response && response.ok) {
            ...
            // EM VEZ DE RELOAD: Chamamos a atualização viva para refletir a mudança na hora
            setTimeout(() => {
                statusModal.hide();
                atualizarListaViva(numeroManifestoAtual); // Atualiza a tela sem piscar
            }, 1500);

        } else {
            throw new Error('Erro no servidor'); // Força a ida para o catch para salvar no DB
        }

    } catch (err) {
        console.warn("📡 Sem rede ou erro no servidor. Salvando no IndexedDB...", err);
        ...
        try {
            ...
            // Salvamos no banco local para o Background Sync enviar depois
            await store.put({
                chaveNF: chave,
                dados: dadosBaixa,
                tipo: 'operacional', // Identificador para o Sync saber qual API chamar
                tentativas: 0
            });

            atualizarStatusUI('warning', '📡 Modo Offline', 'Sem sinal! A baixa será enviada assim que a internet voltar.');
            
            // Fecha o modal e atualiza a lista para mostrar o card como "Sincronizando"
            setTimeout(() => {
                statusModal.hide();
                atualizarListaViva(numeroManifestoAtual);
            }, 2000);
