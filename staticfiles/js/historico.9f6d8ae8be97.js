async function abrirHistorico() {
    ...
    try {
        ...
        // Proteção contra erro de formato
        if (!Array.isArray(manifestos)) throw new Error("Formato de dados inválido");

        if (manifestos.length === 0) {
            container.innerHTML = '<p class="text-center p-4">Nenhum histórico encontrado.</p>';
            return;
        }
