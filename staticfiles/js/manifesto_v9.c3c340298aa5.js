...
async function abrirScanner() {
    // 1. Verificação de permissão e suporte
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert("Seu navegador não suporta acesso à câmera ou você não está em uma conexão segura (HTTPS).");
        return;
    }

    try {
        // Solicita permissão explicitamente antes de abrir o modal
        const stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: "environment" } });
        
        // Se chegou aqui, a permissão foi concedida. Paramos o stream temporário para o ZXing assumir.
        stream.getTracks().forEach(track => track.stop());

        // 2. Abre o Modal e Inicia o Scanner
        ...
        const constraints = {
    video: {
        facingMode: "environment", // Remova o 'exact'
        width: { ideal: 1280 },
        height: { ideal: 720 }
    }
};

        codeReader.decodeFromConstraints(constraints, 'video-scanner', (result, err) => {
            if (result) {
                const codigoLimpo = result.text.replace(/\D/g, ''); 
                const inputBusca = document.getElementById('input-busca-nfe');
                
                inputBusca.value = codigoLimpo;
                filtrarNotasOffline(); // Filtra as notas automaticamente
                
                if (navigator.vibrate) navigator.vibrate(200);
                fecharEScanear(modal);
            }
        });
        ...
        if (err.name === 'NotAllowedError') {
            alert("Acesso negado. Você precisa permitir o uso da câmera nas configurações do seu navegador para escanear.");
        } else {
            alert("Não foi possível acessar a câmera traseira. Verifique se ela não está sendo usada por outro app.");
        }
    }
}
function fecharEScanear(modalInstance) {
    pararScanner();
    modalInstance.hide();
}
...
function pararScanner() {
    if (codeReader) {
        codeReader.reset(); // Desliga a câmera
        codeReader = null;
    }
}
...
async function lerCodigoBarra(input) {
    ...
    try {
        ...
        // 1. Insere o código lido (Chave de Acesso) no campo de busca
        const campoBusca = document.getElementById('input-busca-nfe');
        campoBusca.value = result.text;
        
        // 2. Dispara a filtragem imediata
        filtrarNotasOffline();
        
        // 3. Feedback sonoro ou vibratório (opcional em PWA)
        if (navigator.vibrate) navigator.vibrate(100);

    } catch (err) {
        alert("Não foi possível ler o código de barras. Tente aproximar mais a câmera ou digitar o número da NF.");
        console.error("Erro leitura:", err);
    } finally {
        btnCamera.innerHTML = iconeOriginal;
        URL.revokeObjectURL(imageUrl); // Limpa memória
    }
}