...
async function ligarCameraFull() {
    ...
    try {
        ...
        overlay.style.display = 'block'; // Mostra a tela preta com o vídeo
    } catch (err) {
...
function capturarFotoFull() {
    const video = document.getElementById('video-full');
    const canvas = document.getElementById('canvas-preview'); // Canvas dentro do modal
    const ctx = canvas.getContext('2d');

    // Transfere a imagem do vídeo em tela cheia para o preview do modal
    canvas.width = video.videoWidth;
    canvas.height = video.videoHeight;
    ctx.drawImage(video, 0, 0);

    // Mostra o preview no modal e fecha a câmera
...
function abrirModalBaixa(numeroNota, chaveAcesso) {
    ...
    // Armazena a KEY indispensável para a ESL
...
function resetarInterfaceCamera() {
    // 1. Verificamos apenas os elementos que REALMENTE estão no novo HTML
    const canvas = document.getElementById('canvas-preview');
    const placeholder = document.getElementById('placeholder-camera');
    const inputRecebedor = document.getElementById('input-recebedor');

    // 2. Resetamos o estado para: Mostrar placeholder e esconder o canvas da foto anterior
    if (canvas) canvas.style.display = 'none';
    if (placeholder) placeholder.style.display = 'block';
    if (inputRecebedor) inputRecebedor.value = '';
    
    // O erro ocorria aqui porque tentávamos dar .style.display em algo que não existe mais
    console.log("Interface do modal resetada para nova captura.");
}