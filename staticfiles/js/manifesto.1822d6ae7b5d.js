...
// CONFIGURAÇÕES
// =====================================================
const BASE_API_URL = 'http://localhost:8089/api/';
const ENDPOINTS = {
    busca: `${BASE_API_URL}manifesto/busca/`,
    iniciar: `${BASE_API_URL}manifesto/iniciar/` // endpoint que dispara processa_manifesto_dataexport
};
const LOGIN_URL = 'http://localhost:8089/app/login/';
...
// WebSocket
// =====================================================
function initWebSocket(manifestoId) {
    if (!manifestoId) {
        console.log("Manifesto ID não disponível, WebSocket não iniciado");
        return;
    }
    const ws = new WebSocket(`ws://localhost:8099/ws/manifesto/${manifestoId}/`);

    ws.onopen = () => console.log("WebSocket conectado");
    ws.onmessage = (event) => console.log("Mensagem recebida:", event.data);
    ws.onerror = (event) => console.log("WebSocket erro:", event);
...
async function handleManifestoSearch(event) {
    ...
    try {
        ...
        // Task disparada no backend → WS vai notificar quando terminar
    } catch (err) {
        loadingModal?.hide();
