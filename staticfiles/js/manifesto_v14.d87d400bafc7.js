...
async function sincronizarBaixasPendentes() {
    try {
        ...
        console.log(`🔄 Sincronizando ${pendentes.length} baixas pendentes...`);

        for (const item of pendentes) {
            // Reconstrói o FormData (Blobs/Files não podem ser serializados simples, mas salvamos o Blob no DB)
            const formData = new FormData();
            for (const key in item.campos) {
                formData.append(key, item.campos[key]);
            }
            ...
            if (response.ok) {
                ...
                delTrans.objectStore('baixas_pendentes').delete(item.id);
                console.log(`✅ Nota ${item.numeroNF} sincronizada com sucesso!`);
            }
        }
    } catch (err) {
        console.warn("Sincronização falhou: Ainda sem internet ou erro no servidor.");
    }
}
...
async function salvarRegistro() {
    ...
    let fotoBlob = null;
    if (!isRetida && temFoto) {
        fotoBlob = await new Promise(res => canvas.toBlob(res, 'image/jpeg', 0.70)); // Otimizado para 0.70 para ser mais rápido
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Se o servidor retornar erro controlado (ex: 400, 500)
        ...
        // --- MÁGICA DO MODO OFFLINE ---
        console.warn("Falha de conexão detectada. Salvando no banco de dados interno...");

        try {
            const db = await abrirDB(); // Abre o IndexedDB
            const transaction = db.transaction('baixas_pendentes', 'readwrite');
            const store = transaction.objectStore('baixas_pendentes');

            // Criamos um objeto simples para salvar. 
            // Nota: FormData não pode ser salvo direto em alguns browsers, então salvamos os campos.
            const objOffline = {
                ...
                foto: fotoBlob // O Blob pode ser salvo no IndexedDB
            };

            store.add(objOffline);

            atualizarStatusUI('warning', '📡 Modo Offline Ativado', 'Sem sinal de internet! Sua baixa foi guardada no celular e será enviada assim que o sinal voltar.');
            
            // Mesmo sem internet, a nota SOME para o motorista continuar trabalhando
            processarSumiçoNota(numeroNF);

        } catch (dbErr) {
            console.error("Erro crítico ao salvar no DB interno:", dbErr);
            atualizarStatusUI('error', '❌ Erro de Sistema', 'Não foi possível salvar a baixa nem offline. Verifique o espaço no celular.');
        }
    }
}
...
async function atualizarIconeNuvem() {
    ...
    const request = store.count(); // Conta quantas notas tem no banco
    ...
    request.onsuccess = function() {
        ...
        if (totalPendentes > 0) {
            // Nuvem amarela com o número de notas presas
            container.innerHTML = `
                <div class="position-relative animate__animated animate__pulse animate__infinite">
                    <i class="bi bi-cloud-arrow-up-fill text-warning" style="font-size: 1.5rem;"></i>
                    <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger" style="font-size: 0.6rem;">
                        ${totalPendentes}
                    </span>
                </div>`;
        } else {
            // Nuvem verde de tudo OK
            container.innerHTML = `<i class="bi bi-cloud-check-fill text-success" style="font-size: 1.5rem;"></i>`;
        }
    };
}
