...
async function handleManifestoSearch(event) {
    ...
    // Abre o modal com a primeira mensagem
    document.getElementById('loadingMessage').innerText = "Buscando e validando manifesto...";
    loadingModal?.show();

    try {
        // Agora o backend deve disparar a Task que já faz o ciclo completo
...
function startPolling() {
    ...
    pollingInterval = setInterval(async () => {
        try {
            ...
            // CASO 1: Validação concluída, iniciando processamento de notas
            if (data.status === 'AGUARDANDO' || (data.status === 'PROCESSANDO' && data.etapa === 'ENRIQUECIMENTO')) {
                loadingText.innerText = "Manifesto Aprovado! Processando notas...";
            }
            ...
            if (data.status === 'PROCESSADO') {
                stopPolling();
                loadingModal?.hide();
                // Vai direto para a lista, sem passar pelo preview de "Iniciar"
                renderListaEntregas(manifestoAtual);
            } 
            
            // CASO 3: Erro (CPF divergente, etc)
