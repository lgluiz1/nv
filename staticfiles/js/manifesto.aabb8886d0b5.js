...
function startPolling() {
    ...
    pollingInterval = setInterval(async () => {
        try {
            ...
            // CASO 1: Sucesso (Dono do manifesto validado)
            ...
            // CASO 2: Erro de Validação (CPF Divergente, por exemplo)
            else if (data.status === 'ERRO') {
                stopPolling();
                loadingModal?.hide();
                // Mostra o erro exato que veio da Task (ex: "Manifesto pertence ao documento X")
                renderSearchScreen(data.mensagem_erro || 'Erro na validação do manifesto', 'error');
            }
        } catch (err) { 
