...
function startPolling() {
    ...
    pollingInterval = setInterval(async () => {
        try {
            ...
            // 1. Enquanto a Task do Celery estiver rodando (independente de quantas notas)
            ...
            // 2. O Celery avisará quando terminar a ÚLTIMA nota (seja a 5ª ou a 50ª)
            if (data.status === 'PROCESSADO') {
                ...
                if (contador) {
                    ...
                    contador.innerText = "✅ Todas as notas carregadas!";
                }

                // Feedback visual de finalização
                const listaContainer = document.getElementById('lista-notas-container');
                if (listaContainer) {
                    // Remove o spinner de "buscando mais" se ele existir
                    const spinnerFinal = listaContainer.querySelector('.spinner-border');
                    if (spinnerFinal) spinnerFinal.parentElement.remove();
                }

                setTimeout(() => {
                    window.location.reload(); 
                }, 2000); // 2 segundos para manifestos grandes serem apreciados
            } 
