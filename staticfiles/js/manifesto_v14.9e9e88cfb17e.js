...
async function atualizarListaViva(numeroManifesto) {
    try {
        ...
        // --- 2. ANÁLISE DE MUDANÇA (SILENT REFRESH) ---
        ...
        if (areaDinamica && notas.length > 0) {
            ...
            notas.forEach(nf => {
                ...
                if (baixada) {
                    totalFinalizadas++;
                    // Passamos 'false' para sincronizando em notas já baixadas
            ...
            // 3. RENDERIZAÇÃO DAS NOTAS PENDENTES (AGRUPADAS) - MANTIDO ORIGINAL COM SYNC
            let htmlPendentes = '';
            Object.keys(grupos).forEach(tipo => {
                if (grupos[tipo].length > 0) {
                    ...
                    grupos[tipo].forEach(nf => {
                        // --- VERIFICA SE ESTÁ NO LIMBO ANTES DE RENDERIZAR ---
            ...
            // --- RESTANTE DA LÓGICA ORIGINAL (COLETIVA, CONTADORES, KM FINAL) ---
            ...
            // --- 4. SALVAMENTO DO CACHE FINAL ---
            if (areaDinamica.innerHTML.length > 100) {
