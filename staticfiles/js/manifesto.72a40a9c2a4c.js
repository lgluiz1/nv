...
async function ligarCamera() {
    try {
        ...
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            alert("O seu navegador não suporta acesso à câmera ou você não está em uma conexão segura (HTTPS).");
            return;
        }

        // Tenta solicitar permissão e abrir a câmera traseira
        const constraints = { 
        ...
        if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
            alert("Acesso negado. Por favor, permita o uso da câmera nas configurações do seu navegador.");
        } else {
            // Tenta qualquer câmera se a traseira falhar
            tentarQualquerCamera();
        }
    }
}
