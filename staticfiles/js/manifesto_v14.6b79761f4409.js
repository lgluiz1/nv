...
async function atualizarListaViva(numeroManifesto) {
    try {
        ...
        // 1. BUSCA NOTAS QUE ESTÃO NO LIMBO (INDEXEDDB) - NOVO
        const db = await abrirDB();
        ...
        // 2. ANÁLISE DE MUDANÇA PARA EVITAR O "PISCADO" (SILENT REFRESH) - NOVO
        ...
        if (areaDinamica && notas.length > 0) {
            // --- INJEÇÃO DA BUSCA (Se não existir) - MANTIDO SEU ORIGINAL ---
            ...
            // 2. SEPARAÇÃO DAS NOTAS (MANTIDO SEU ORIGINAL)
            notas.forEach(nf => {
                ...
                if (baixada) {
                    totalFinalizadas++;
                    // Aqui mantemos sua função original com o novo parâmetro "false" para sincronizando
            ...
            // 3. RENDERIZAÇÃO DAS NOTAS PENDENTES (MANTIDO SEU ORIGINAL COM AJUSTE DE SYNC)
            let htmlPendentes = '';
            Object.keys(grupos).forEach(tipo => {
                if (grupos[tipo].length > 0) {
                    ...
                    grupos[tipo].forEach(nf => {
                        // VERIFICA SE ESTÁ SINCRONIZANDO OFF-LINE
            ...
            // 4. EXIBIÇÃO DOS CONCLUÍDOS (MANTIDO ORIGINAL)
            ...
            // --- LÓGICA DE BAIXA COLETIVA E CONTADORES (MANTIDO 100% ORIGINAL) ---
